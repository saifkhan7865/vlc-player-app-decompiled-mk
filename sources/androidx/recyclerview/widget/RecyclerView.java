...
public class RecyclerView extends ViewGroup implements ScrollingView, NestedScrollingChild2, NestedScrollingChild3 {
    ...
    public String exceptionLabel() {
        return AnsiRenderer.CODE_TEXT_SEPARATOR + super.toString() + ", adapter:" + this.mAdapter + ", layout:" + this.mLayout + ", context:" + getContext();
    }
    ...
    private boolean shouldAbsorb(EdgeEffect edgeEffect, int i, int i2) {
        ...
        if (getSplineFlingDistance(-i) < EdgeEffectCompat.getDistance(edgeEffect) * ((float) i2)) {
            return true;
        }
        return false;
    }
    ...
    private int consumeFlingInStretch(int i, EdgeEffect edgeEffect, EdgeEffect edgeEffect2, int i2) {
        if (i > 0 && edgeEffect != null && EdgeEffectCompat.getDistance(edgeEffect) != 0.0f) {
            int round = Math.round((((float) (-i2)) / FLING_DESTRETCH_FACTOR) * EdgeEffectCompat.onPullDistance(edgeEffect, (((float) (-i)) * FLING_DESTRETCH_FACTOR) / ((float) i2), 0.5f));
            if (round != i) {
                edgeEffect.finish();
            }
            return i - round;
            ...
            float f = (float) i2;
            int round2 = Math.round((f / FLING_DESTRETCH_FACTOR) * EdgeEffectCompat.onPullDistance(edgeEffect2, (((float) i) * FLING_DESTRETCH_FACTOR) / f, 0.5f));
            if (round2 != i) {
                edgeEffect2.finish();
            }
            return i - round2;
    ...
    private void pullGlows(float f, float f2, float f3, float f4) {
        ...
        if (f2 < 0.0f) {
            ensureLeftGlow();
            EdgeEffectCompat.onPullDistance(this.mLeftGlow, (-f2) / ((float) getWidth()), 1.0f - (f3 / ((float) getHeight())));
        } else if (f2 > 0.0f) {
            ensureRightGlow();
            EdgeEffectCompat.onPullDistance(this.mRightGlow, f2 / ((float) getWidth()), f3 / ((float) getHeight()));
        } else {
            z = false;
        }
        if (f4 < 0.0f) {
            ensureTopGlow();
            EdgeEffectCompat.onPullDistance(this.mTopGlow, (-f4) / ((float) getHeight()), f / ((float) getWidth()));
        } else if (f4 > 0.0f) {
            ensureBottomGlow();
            EdgeEffectCompat.onPullDistance(this.mBottomGlow, f4 / ((float) getHeight()), 1.0f - (f / ((float) getWidth())));
    ...
    private float getSplineFlingDistance(int i) {
        ...
        double d = (double) f;
        Double.isNaN(d);
        double d2 = (double) (this.mPhysicalCoef * SCROLL_FRICTION);
        double d3 = (double) f;
        Double.isNaN(d3);
    ...
    class ViewFlinger implements Runnable {
        ...
        public void run() {
            ...
            if (overScroller.computeScrollOffset()) {
                ...
                if (recyclerView.dispatchNestedPreScroll(consumeFlingInHorizontalStretch, consumeFlingInVerticalStretch, recyclerView.mReusableIntPair, (int[]) null, 1)) {
                ...
                recyclerView3.dispatchNestedScroll(i2, i, consumeFlingInHorizontalStretch, consumeFlingInVerticalStretch, (int[]) null, 1, recyclerView3.mReusableIntPair);
    ...
    public final class Recycler {
        ...
        public boolean validateViewHolderForOffsetPosition(ViewHolder viewHolder) {
            ...
            } else if (viewHolder.mPosition < 0 || viewHolder.mPosition >= RecyclerView.this.mAdapter.getItemCount()) {
        ...
        public void recycleViewHolderInternal(ViewHolder viewHolder) {
            ...
            if (viewHolder.isScrap() || viewHolder.itemView.getParent() != null) {
                ...
                boolean z4 = RecyclerView.this.mAdapter != null && doesTransientStatePreventRecycling && RecyclerView.this.mAdapter.onFailedToRecycleView(viewHolder);
