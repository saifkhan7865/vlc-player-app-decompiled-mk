...
class ProfileTranscoder {
    ...
    private static DexProfileData[] readUncompressedBody(InputStream inputStream, String str, int i) throws IOException {
        ...
        for (int i3 = 0; i3 < i2; i3++) {
            ...
            dexProfileDataArr[i3] = new DexProfileData(str2, Encoding.readString(inputStream2, readUInt16), Encoding.readUInt32(inputStream), 0, readUInt162, (int) readUInt32, (int) Encoding.readUInt32(inputStream), iArr, new TreeMap());
        }
    ...
    private static void readMethodBitmap(InputStream inputStream, DexProfileData dexProfileData) throws IOException {
        BitSet m = BitSet.valueOf(Encoding.read(inputStream, Encoding.bitsToBytes(dexProfileData.numMethodIds * 2)));
        for (int i = 0; i < dexProfileData.numMethodIds; i++) {
            int readFlagsFromBitmap = readFlagsFromBitmap(m, i, dexProfileData.numMethodIds);
