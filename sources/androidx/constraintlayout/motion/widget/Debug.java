...
public class Debug {
    public static void logStack(String str, String str2, int i) {
        ...
        String str3 = AnsiRenderer.CODE_TEXT_SEPARATOR;
    ...
    public static void printStack(String str, int i) {
        StackTraceElement[] stackTrace = new Throwable().getStackTrace();
        int min = Math.min(i, stackTrace.length - 1);
        String str2 = AnsiRenderer.CODE_TEXT_SEPARATOR;
        for (int i2 = 1; i2 <= min; i2++) {
            StackTraceElement stackTraceElement = stackTrace[i2];
        ...
        for (Field field : fields) {
            try {
                ...
                if (field.getName().startsWith("layout_constraint")) {
                    if (!(obj2 instanceof Integer) || !obj2.toString().equals(Constants.GROUP_VIDEOS_NONE)) {
                        if (!(obj2 instanceof Integer) || !obj2.toString().equals(Constants.GROUP_VIDEOS_FOLDER)) {
                            if (!(obj2 instanceof Float) || !obj2.toString().equals("1.0")) {
                                if (!(obj2 instanceof Float) || !obj2.toString().equals("0.5")) {
                                    System.out.println(str + "    " + field.getName() + AnsiRenderer.CODE_TEXT_SEPARATOR + obj2);
                                }
                            }
                        }
                    }
    ...
    /* JADX WARNING: Code restructure failed: missing block: B:14:?, code lost:
        r0 = "? " + r6[r1] + org.fusesource.jansi.AnsiRenderer.CODE_TEXT_SEPARATOR;
     */
            ...
            for (Field field : fields) {
                try {
                    ...
                    if (field.getName().contains("To")) {
                        if (!obj.toString().equals(Constants.GROUP_VIDEOS_NONE)) {
                            System.out.println(str2 + "       " + field.getName() + AnsiRenderer.CODE_TEXT_SEPARATOR + obj);
                        }
                    }
                } catch (IllegalAccessException unused) {
                }
    ...
    public static void dumpLayoutParams(ViewGroup.LayoutParams layoutParams, String str) {
        ...
        for (Field field : fields) {
            try {
                ...
                if (name.contains("To")) {
                    if (!obj.toString().equals(Constants.GROUP_VIDEOS_NONE)) {
                        System.out.println(str2 + "       " + name + AnsiRenderer.CODE_TEXT_SEPARATOR + obj);
                    }
                }
            } catch (IllegalAccessException unused) {
            }
