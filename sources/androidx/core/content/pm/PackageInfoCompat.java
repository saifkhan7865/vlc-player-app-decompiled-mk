...
public final class PackageInfoCompat {
    ...
    public static List<Signature> getSignatures(PackageManager packageManager, String str) throws PackageManager.NameNotFoundException {
        ...
        if (Build.VERSION.SDK_INT >= 28) {
            SigningInfo m = packageManager.getPackageInfo(str, 134217728).signingInfo;
            if (Api28Impl.hasMultipleSigners(m)) {
                signatureArr = Api28Impl.getApkContentsSigners(m);
            } else {
                signatureArr = Api28Impl.getSigningCertificateHistory(m);
            }
        } else {
