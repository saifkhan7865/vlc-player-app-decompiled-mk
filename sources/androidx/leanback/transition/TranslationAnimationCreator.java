...
class TranslationAnimationCreator {
    static Animator createAnimation(View view, TransitionValues transitionValues, int i, int i2, float f, float f2, float f3, float f4, TimeInterpolator timeInterpolator, Transition transition) {
        ...
        int[] iArr = (int[]) FragmentUtil$$ExternalSyntheticApiModelOutline0.m(transitionValues).getTag(R.id.transitionPosition);
        if (iArr != null) {
        ...
        path.moveTo(f, f2);
        path.lineTo(f3, f4);
        ObjectAnimator m = ObjectAnimator.ofFloat(view, View.TRANSLATION_X, View.TRANSLATION_Y, path);
        TransitionPositionListener transitionPositionListener = new TransitionPositionListener(view, FragmentUtil$$ExternalSyntheticApiModelOutline0.m(transitionValues), round, round2, translationX, translationY);
        Transition unused = transition.addListener(transitionPositionListener);
        m.addListener(transitionPositionListener);
        m.addPauseListener(transitionPositionListener);
