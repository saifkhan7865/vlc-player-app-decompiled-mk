...
public class RainbowSigner implements MessageSigner {
    ...
    private short[] initSign(Layer[] layerArr, short[] sArr) {
        ...
        for (int i = 0; i < layerArr[0].getVi(); i++) {
            ...
            short[] sArr3 = this.x;
            sArr3[i] = (short) (sArr3[i] & Http2CodecUtil.MAX_UNSIGNED_BYTE);
        }
    ...
    private short[] makeMessageRepresentative(byte[] bArr) {
        ...
        while (i2 < bArr.length) {
            ...
            sArr[i2] = s;
            sArr[i2] = (short) (s & Http2CodecUtil.MAX_UNSIGNED_BYTE);
            i3++;
            i2++;
            if (i2 >= i) {
                break;
    ...
    public boolean verifySignature(byte[] bArr, byte[] bArr2) {
        ...
        for (int i = 0; i < bArr2.length; i++) {
            sArr[i] = (short) (((short) bArr2[i]) & Http2CodecUtil.MAX_UNSIGNED_BYTE);
        }
        short[] makeMessageRepresentative = makeMessageRepresentative(bArr);
