...
class OcspCache {
    ...
    private static Map<URI, WeakReference<Map<CertID, OCSPResponse>>> cache = DesugarCollections.synchronizedMap(new WeakHashMap());
    ...
    static OCSPResponse getOcspResponse(CertID certID, PKIXCertRevocationCheckerParameters pKIXCertRevocationCheckerParameters, URI uri, X509Certificate x509Certificate, List<Extension> list, JcaJceHelper jcaJceHelper) throws CertPathValidatorException {
        ...
        try {
            ...
            aSN1EncodableVector.add(new Request(certID, (Extensions) null));
            ASN1EncodableVector aSN1EncodableVector2 = new ASN1EncodableVector();
            byte[] bArr = null;
            for (int i2 = 0; i2 != list.size(); i2++) {
                Extension m = AppUtils$$ExternalSyntheticApiModelOutline0.m((Object) list.get(i2));
                byte[] m2 = AppUtils$$ExternalSyntheticApiModelOutline0.m(m);
                if (OCSPObjectIdentifiers.id_pkix_ocsp_nonce.getId().equals(AppUtils$$ExternalSyntheticApiModelOutline0.m(m))) {
                    bArr = m2;
                }
                aSN1EncodableVector2.add(new org.bouncycastle.asn1.x509.Extension(new ASN1ObjectIdentifier(AppUtils$$ExternalSyntheticApiModelOutline0.m(m)), AppUtils$$ExternalSyntheticApiModelOutline0.m(m), m2));
            }
            try {
                ...
                if (instance2.getResponseStatus().getIntValue() == 0) {
                    ...
                    if (instance3.getResponseType().equals((ASN1Primitive) OCSPObjectIdentifiers.id_pkix_ocsp_basic)) {
                        try {
                            if (ProvOcspRevocationChecker.validatedOcspResponse(BasicOCSPResponse.getInstance(instance3.getResponse().getOctets()), pKIXCertRevocationCheckerParameters, bArr, x509Certificate, jcaJceHelper)) {
                                WeakReference weakReference2 = cache.get(uri2);
                                if (weakReference2 != null) {
                                    ((Map) weakReference2.get()).put(certID, instance2);
                                } else {
