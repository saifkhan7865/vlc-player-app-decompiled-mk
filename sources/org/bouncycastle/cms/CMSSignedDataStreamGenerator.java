...
public class CMSSignedDataStreamGenerator extends CMSSignedGenerator {
    ...
    private ASN1Integer calculateVersion(ASN1ObjectIdentifier aSN1ObjectIdentifier) {
        ...
        return z ? new ASN1Integer(4) : z2 ? new ASN1Integer(3) : checkForVersion3(this._signers, this.signerGens) ? new ASN1Integer(3) : !CMSObjectIdentifiers.data.equals((ASN1Primitive) aSN1ObjectIdentifier) ? new ASN1Integer(3) : new ASN1Integer(1);
    }
    ...
    public List<AlgorithmIdentifier> getDigestAlgorithms() {
        ...
        for (SignerInformation digestAlgorithmID : this._signers) {
            arrayList.add(CMSSignedHelper.INSTANCE.fixDigestAlgID(digestAlgorithmID.getDigestAlgorithmID(), this.digestAlgIdFinder));
        }
    ...
    public OutputStream open(ASN1ObjectIdentifier aSN1ObjectIdentifier, OutputStream outputStream, boolean z, OutputStream outputStream2) throws IOException {
        ...
        for (SignerInformation addDigestAlgs : this._signers) {
            CMSUtils.addDigestAlgs(hashSet, addDigestAlgs, this.digestAlgIdFinder);
        }
        for (SignerInfoGenerator digestAlgorithm : this.signerGens) {
            hashSet.add(digestAlgorithm.getDigestAlgorithm());
        }
