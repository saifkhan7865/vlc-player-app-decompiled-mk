...
class CMSEnvelopedHelper {
    ...
    static class CMSDigestAuthenticatedSecureReadable implements CMSSecureReadable {
        ...
        public CMSDigestAuthenticatedSecureReadable(DigestCalculator digestCalculator2, ASN1ObjectIdentifier aSN1ObjectIdentifier, CMSReadable cMSReadable) {
    ...
    private static void readRecipientInfo(List list, RecipientInfo recipientInfo, AlgorithmIdentifier algorithmIdentifier, CMSSecureReadable cMSSecureReadable, AuthAttributesProvider authAttributesProvider) {
        ...
        if (info instanceof KeyTransRecipientInfo) {
            ...
            OtherRecipientInfo instance = OtherRecipientInfo.getInstance(info);
            if (CMSObjectIdentifiers.id_ori_kem.equals((ASN1Primitive) instance.getType())) {
                passwordRecipientInformation = new KEMRecipientInformation(KEMRecipientInfo.getInstance(instance.getValue()), algorithmIdentifier, cMSSecureReadable, authAttributesProvider);
            } else {
                return;
            }
