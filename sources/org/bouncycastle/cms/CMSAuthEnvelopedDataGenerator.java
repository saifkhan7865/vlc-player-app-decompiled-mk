    ...
    private CMSAuthEnvelopedData doGenerate(CMSTypedData cMSTypedData, OutputAEADEncryptor outputAEADEncryptor) throws CMSException {
        ...
        try {
            ...
            if (this.unauthAttrsGenerator != null) {
                dLSet = new DLSet(this.unauthAttrsGenerator.getAttributes(Collections.EMPTY_MAP).toASN1EncodableVector());
            }
            return new CMSAuthEnvelopedData(new ContentInfo(CMSObjectIdentifiers.authEnvelopedData, new AuthEnvelopedData(this.originatorInfo, new DERSet(aSN1EncodableVector), encryptedContentInfo, dERSet, new DEROctetString(mac), dLSet)));
        } catch (IOException e) {
