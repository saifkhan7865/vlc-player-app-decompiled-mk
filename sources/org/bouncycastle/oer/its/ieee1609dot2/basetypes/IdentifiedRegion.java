...
public class IdentifiedRegion extends ASN1Object implements ASN1Choice, RegionInterface {
    ...
    private IdentifiedRegion(ASN1TaggedObject aSN1TaggedObject) {
        ...
        this.choice = tagNo;
        if (tagNo == 0) {
            aSN1Encodable = CountryOnly.getInstance(aSN1TaggedObject.getExplicitBaseObject());
        } else if (tagNo == 1) {
            aSN1Encodable = CountryAndRegions.getInstance(aSN1TaggedObject.getExplicitBaseObject());
        } else if (tagNo == 2) {
            aSN1Encodable = CountryAndSubregions.getInstance(aSN1TaggedObject.getExplicitBaseObject());
        } else {
