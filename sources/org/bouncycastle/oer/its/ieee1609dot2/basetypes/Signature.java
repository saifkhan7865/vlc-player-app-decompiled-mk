...
public class Signature extends ASN1Object implements ASN1Choice {
    ...
    private Signature(ASN1TaggedObject aSN1TaggedObject) {
        ...
        this.choice = tagNo;
        if (tagNo == 0 || tagNo == 1) {
            aSN1Encodable = EcdsaP256Signature.getInstance(aSN1TaggedObject.getExplicitBaseObject());
        } else if (tagNo == 2) {
            aSN1Encodable = EcdsaP384Signature.getInstance(aSN1TaggedObject.getExplicitBaseObject());
        } else {
