...
public class GeographicRegion extends ASN1Object implements ASN1Choice {
    ...
    private GeographicRegion(ASN1TaggedObject aSN1TaggedObject) {
        ...
        this.choice = tagNo;
        if (tagNo == 0) {
            aSN1Encodable = CircularRegion.getInstance(aSN1TaggedObject.getExplicitBaseObject());
        } else if (tagNo == 1) {
            aSN1Encodable = SequenceOfRectangularRegion.getInstance(aSN1TaggedObject.getExplicitBaseObject());
        } else if (tagNo == 2) {
            aSN1Encodable = PolygonalRegion.getInstance(aSN1TaggedObject.getExplicitBaseObject());
        } else if (tagNo == 3) {
            aSN1Encodable = SequenceOfIdentifiedRegion.getInstance(aSN1TaggedObject.getExplicitBaseObject());
        } else {
