...
public class EccP384CurvePoint extends EccCurvePoint implements ASN1Choice {
    ...
    private EccP384CurvePoint(ASN1TaggedObject aSN1TaggedObject) {
        ...
        if (tagNo != 0) {
            if (tagNo == 1) {
                aSN1Encodable = ASN1Null.getInstance(aSN1TaggedObject.getExplicitBaseObject());
            } else if (!(tagNo == 2 || tagNo == 3)) {
                if (tagNo == 4) {
                    aSN1Encodable = ASN1Sequence.getInstance(aSN1TaggedObject.getExplicitBaseObject());
                } else {
        ...
        aSN1Encodable = ASN1OctetString.getInstance(aSN1TaggedObject.getExplicitBaseObject());
        this.eccP384CurvePoint = aSN1Encodable;
    }
    ...
    public byte[] getEncodedPoint() {
        ...
        if (i != 0) {
            if (i == 2) {
                ...
                ASN1Sequence instance = ASN1Sequence.getInstance(this.eccP384CurvePoint);
                return Arrays.concatenate(new byte[]{4}, DEROctetString.getInstance(instance.getObjectAt(0)).getOctets(), DEROctetString.getInstance(instance.getObjectAt(1)).getOctets());
            } else {
