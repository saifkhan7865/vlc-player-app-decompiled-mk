...
public class CertificateId extends ASN1Object implements ASN1Choice {
    ...
    private CertificateId(ASN1TaggedObject aSN1TaggedObject) {
        ...
        this.choice = tagNo;
        if (tagNo == 0) {
            aSN1Encodable = LinkageData.getInstance(aSN1TaggedObject.getExplicitBaseObject());
        } else if (tagNo == 1) {
            aSN1Encodable = Hostname.getInstance(aSN1TaggedObject.getExplicitBaseObject());
        } else if (tagNo == 2) {
            aSN1Encodable = DEROctetString.getInstance(aSN1TaggedObject.getExplicitBaseObject());
        } else if (tagNo == 3) {
