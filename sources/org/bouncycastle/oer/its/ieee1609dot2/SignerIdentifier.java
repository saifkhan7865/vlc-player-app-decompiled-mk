...
public class SignerIdentifier extends ASN1Object implements ASN1Choice {
    ...
    private SignerIdentifier(ASN1TaggedObject aSN1TaggedObject) {
        ...
        this.choice = tagNo;
        if (tagNo == 0) {
            aSN1Encodable = HashedId8.getInstance(aSN1TaggedObject.getExplicitBaseObject());
        } else if (tagNo == 1) {
            aSN1Encodable = SequenceOfCertificate.getInstance(aSN1TaggedObject.getExplicitBaseObject());
        } else if (tagNo == 2) {
            aSN1Encodable = DERNull.getInstance(aSN1TaggedObject.getExplicitBaseObject());
        } else {
