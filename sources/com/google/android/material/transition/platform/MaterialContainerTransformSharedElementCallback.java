...
public class MaterialContainerTransformSharedElementCallback extends SharedElementCallback {
    ...
    private void setUpEnterTransform(final Window window) {
        Transition m$3 = window.getSharedElementEnterTransition();
        if (m$3 instanceof MaterialContainerTransform) {
            MaterialContainerTransform materialContainerTransform = (MaterialContainerTransform) m$3;
            ...
            if (this.transparentWindowBackgroundEnabled) {
                ...
                Transition unused = materialContainerTransform.addListener(new TransitionListenerAdapter() {
    ...
    private void setUpReturnTransform(final Activity activity, final Window window) {
        Transition m = window.getSharedElementReturnTransition();
        if (m instanceof MaterialContainerTransform) {
            MaterialContainerTransform materialContainerTransform = (MaterialContainerTransform) m;
            ...
            if (this.transparentWindowBackgroundEnabled) {
                updateBackgroundFadeDuration(window, materialContainerTransform);
                Transition unused2 = materialContainerTransform.addListener(new TransitionListenerAdapter() {
                    public void onTransitionStart(Transition transition) {
                        MaterialContainerTransformSharedElementCallback.removeWindowBackground(window);
                    }
                });
    ...
    private static void updateBackgroundFadeDuration(Window window, MaterialContainerTransform materialContainerTransform) {
        if (Chip$$ExternalSyntheticApiModelOutline1.m(materialContainerTransform) >= 0) {
            window.setTransitionBackgroundFadeDuration(Chip$$ExternalSyntheticApiModelOutline1.m(materialContainerTransform));
        }
    }
}
