...
class TransitionUtils {
    ...
    static boolean maybeApplyThemeInterpolator(Transition transition, Context context, int i, TimeInterpolator timeInterpolator) {
        if (i == 0 || Chip$$ExternalSyntheticApiModelOutline1.m(transition) != null) {
            return false;
        }
        Transition unused = transition.setInterpolator(MotionUtils.resolveThemeInterpolator(context, i, timeInterpolator));
        return true;
    }

    static boolean maybeApplyThemeDuration(Transition transition, Context context, int i) {
        int resolveThemeDuration;
        if (i == 0 || Chip$$ExternalSyntheticApiModelOutline1.m(transition) != -1 || (resolveThemeDuration = MotionUtils.resolveThemeDuration(context, i, -1)) == -1) {
            return false;
        }
        Transition unused = transition.setDuration((long) resolveThemeDuration);
        return true;
    }
    ...
    static ShapeAppearanceModel convertToRelativeCornerSizes(ShapeAppearanceModel shapeAppearanceModel, RectF rectF) {
        return shapeAppearanceModel.withTransformedCornerSizes(new TransitionUtils$$ExternalSyntheticLambda4(rectF));
    }
    ...
    static void maybeAddTransition(TransitionSet transitionSet, Transition transition) {
        if (transition != null) {
            TransitionSet unused = transitionSet.addTransition(transition);
        }
    }

    static void maybeRemoveTransition(TransitionSet transitionSet, Transition transition) {
        if (transition != null) {
            TransitionSet unused = transitionSet.removeTransition(transition);
        }
    }
}
