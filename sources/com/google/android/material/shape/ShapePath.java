    ...
    static class InnerCornerShadowOperation extends ShadowCompatOperation {
        ...
        public void draw(Matrix matrix, ShadowRenderer shadowRenderer, int i, Canvas canvas) {
            ...
            if (sweepAngle <= 0.0f) {
                ...
                double d2 = tan * d;
                if (hypot > d2) {
                    RectF rectF = new RectF(0.0f, 0.0f, (float) (hypot - d2), 0.0f);
                    this.renderMatrix.set(matrix2);
                    this.renderMatrix.preTranslate(this.startX, this.startY);
                    this.renderMatrix.preRotate(getStartAngle());
                    shadowRenderer.drawEdgeShadow(canvas2, this.renderMatrix, rectF, i2);
                } else {
                ...
                Double.isNaN(d);
                matrix3.preTranslate((float) ((-d2) - d), -2.0f * min);
                Matrix matrix4 = this.renderMatrix;
                Double.isNaN(d);
                float[] fArr = {(float) (d + d2), f};
                int i3 = (int) min;
                double d3 = d2;
                shadowRenderer.drawInnerCornerShadow(canvas, matrix4, rectF2, i3, 450.0f, sweepAngle, fArr);
                if (hypot2 > d3) {
