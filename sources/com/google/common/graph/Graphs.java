...
public final class Graphs {
        ...
        for (N1 next : mutableValueGraph.nodes()) {
            for (N next2 : valueGraph.successors(next)) {
                if (mutableValueGraph.nodes().contains(next2)) {
                    mutableValueGraph.putEdgeValue(next, next2, j$.util.Objects.requireNonNull(valueGraph.edgeValueOrDefault(next, next2, null)));
                }
            }
        }
    ...
    public static <N, V> MutableValueGraph<N, V> copyOf(ValueGraph<N, V> valueGraph) {
        ...
        for (EndpointPair next : valueGraph.edges()) {
            build.putEdgeValue(next.nodeU(), next.nodeV(), j$.util.Objects.requireNonNull(valueGraph.edgeValueOrDefault(next.nodeU(), next.nodeV(), null)));
        }
        return build;
    }
