...
final class StandardMutableValueGraph<N, V> extends StandardValueGraph<N, V> implements MutableValueGraph<N, V> {
    ...
    public boolean removeNode(N n) {
        ...
        for (Object withoutCaching : graphConnections.successors()) {
            ((GraphConnections) Objects.requireNonNull((GraphConnections) this.nodeConnections.getWithoutCaching(withoutCaching))).removePredecessor(n);
            this.edgeCount--;
        }
        if (isDirected()) {
            for (Object withoutCaching2 : graphConnections.predecessors()) {
                Preconditions.checkState(((GraphConnections) Objects.requireNonNull((GraphConnections) this.nodeConnections.getWithoutCaching(withoutCaching2))).removeSuccessor(n) != null);
                this.edgeCount--;
            }
        }
