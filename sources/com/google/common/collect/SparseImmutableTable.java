...
final class SparseImmutableTable<R, C, V> extends RegularImmutableTable<R, C, V> {
    ...
    SparseImmutableTable(ImmutableList<Table.Cell<R, C, V>> immutableList, ImmutableSet<R> immutableSet, ImmutableSet<C> immutableSet2) {
        ...
        for (int i = 0; i < immutableList.size(); i++) {
            ...
            iArr[i] = ((Integer) Objects.requireNonNull(indexMap.get(rowKey))).intValue();
    ...
    public ImmutableTable.SerializedForm createSerializedForm() {
        ...
        int i = 0;
        while (it.hasNext()) {
            iArr[i] = ((Integer) Objects.requireNonNull((Integer) indexMap.get(((Table.Cell) it.next()).getColumnKey()))).intValue();
            i++;
        }
