...
public final class Base64Kt {
    ...
    public static final String encodeBase64(byte[] bArr) {
        ...
        while (true) {
            ...
            byte b = (bArr[i4 + 2] & 255) | ((bArr[i4] & 255) << Ascii.DLE) | ((bArr[i4 + 1] & 255) << 8);
            int i7 = 3;
            while (-1 < i7) {
        ...
        if (length == 1) {
            i = (bArr[i4] & 255) << Ascii.DLE;
        } else {
            i = ((bArr[i4 + 1] & 255) << 8) | ((bArr[i4] & 255) << Ascii.DLE);
        }
        int i8 = ((3 - length) * 8) / 6;
        if (i8 <= 3) {
            while (true) {
    ...
    public static final Input decodeBase64Bytes(ByteReadPacket byteReadPacket) {
        ...
        try {
            ...
            while (byteReadPacket.getRemaining() > 0) {
                ...
                int i3 = 0;
                while (i < 4) {
                    i2 |= ((byte) (((byte) BASE64_INVERSE_ALPHABET[bArr[i] & 255]) & 63)) << ((3 - i3) * 6);
                    i++;
                    i3++;
                }
                int i4 = 4 - readAvailable$default;
                int i5 = 2;
                if (i4 <= 2) {
                    while (true) {
                        bytePacketBuilder.writeByte((byte) ((i2 >> (i5 * 8)) & 255));
                        if (i5 == i4) {
                            break;
                        }
                        i5--;
