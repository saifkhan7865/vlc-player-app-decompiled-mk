...
final class SpdyCodecUtil {
    ...
    static int getUnsignedMedium(ByteBuf byteBuf, int i) {
        return (byteBuf.getByte(i + 2) & 255) | ((byteBuf.getByte(i) & 255) << Ascii.DLE) | ((byteBuf.getByte(i + 1) & 255) << 8);
    }

    static int getUnsignedInt(ByteBuf byteBuf, int i) {
        return (byteBuf.getByte(i + 3) & 255) | ((byteBuf.getByte(i) & Byte.MAX_VALUE) << Ascii.CAN) | ((byteBuf.getByte(i + 1) & 255) << Ascii.DLE) | ((byteBuf.getByte(i + 2) & 255) << 8);
    }

    static int getSignedInt(ByteBuf byteBuf, int i) {
        return (byteBuf.getByte(i + 3) & 255) | ((byteBuf.getByte(i) & 255) << Ascii.CAN) | ((byteBuf.getByte(i + 1) & 255) << Ascii.DLE) | ((byteBuf.getByte(i + 2) & 255) << 8);
    }
    ...
    static void validateHeaderName(CharSequence charSequence) {
        ObjectUtil.checkNonEmpty(charSequence, ContentDisposition.Parameters.Name);
        if (charSequence.length() <= 65535) {
            int i = 0;
            while (i < charSequence.length()) {
                char charAt = charSequence.charAt(i);
                if (charAt == 0) {
                    throw new IllegalArgumentException("name contains null character: " + charSequence);
                } else if (charAt >= 'A' && charAt <= 'Z') {
                    throw new IllegalArgumentException("name must be all lower case.");
                } else if (charAt <= 127) {
                    i++;
                } else {
                    throw new IllegalArgumentException("name contains non-ascii character: " + charSequence);
                }
            }
            return;
        }
        throw new IllegalArgumentException("name exceeds allowable length: " + charSequence);
    }
