...
final class UnsafeByteBufUtil {
    ...
    static void setZero(byte[] bArr, int i, int i2) {
        if (i2 != 0) {
            PlatformDependent.setMemory(bArr, i, (long) i2, (byte) 0);
        }
    }
    ...
    static void setBytes(AbstractByteBuf abstractByteBuf, long j, int i, ByteBuffer byteBuffer) {
        ...
        if (remaining != 0) {
            if (byteBuffer.isDirect()) {
                ...
                PlatformDependent.copyMemory(PlatformDependent.directBufferAddress(byteBuffer) + ((long) byteBuffer.position()), j, (long) remaining);
                ...
                abstractByteBuf.internalNioBuffer(i, remaining).put(byteBuffer2);
            }
        }
    }
    ...
    static void setZero(long j, int i) {
        if (i != 0) {
            PlatformDependent.setMemory(j, (long) i, (byte) 0);
        }
    }
